...
function main_test()
	local model = torch.load('trained_model/102915-tisse-fate-5-epochs.model')
	local training_dataset, testing_dataset, classes, classes_names = dofile('data.lua')
	local criterion = nn.ClassNLLCriterion()-- based on what we use in train
